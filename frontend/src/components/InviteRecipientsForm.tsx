...
const InviteRecipientsForm: React.FC<InviteRecipientsFormProps> = ({
  ...
  description = 'Optional — invite people by email or phone to join this shared account.'
}) => {
  ...
  return (
    <div style={{
      ...
      <h3 style={{ margin: '0 0 0.25rem 0', fontSize: '1rem', color: '#2d3748' }}>{title}</h3>
      ...
      <div className="form-group" style={{ marginBottom: '1rem' }}>
        ...
        {loadingFriends ? (
          ...
          <p style={{ fontSize: '0.85rem', color: '#718096', margin: 0 }}>
            ...
            <Link to="/friends">Add friends</Link> to pick them quickly when inviting.
          </p>
        )}
      </div>
      ...
      {recipients.map((recipient, index) => (
        <div key={index} style={{ marginBottom: '0.75rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }}>
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#4a5568' }}>Invite {index + 1}</span>
            {recipients.length > 1 && (
              <button
                type="button"
