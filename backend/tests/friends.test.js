...
describe('Friends API Endpoints', () => {
  ...
  describe('POST /api/friends', () => {
    ...
    it('should reject duplicate friend relationships', async () => {
      await request(app)
        .post('/api/friends')
        .set('Authorization', `Bearer ${tokenA}`)
        .send({ email: 'bob@test.com' })
  ...
  describe('DELETE /api/friends/:friendId', () => {
    it('should remove a friend from the list', async () => {
      ...
      const stillExists = await User.findById(userB._id);
      expect(stillExists).not.toBeNull();
    });

    it('should reject invalid friend IDs', async () => {
      await request(app)
        .delete('/api/friends/not-a-valid-id')
        .set('Authorization', `Bearer ${tokenA}`)
        .expect(400);
    });
  });
